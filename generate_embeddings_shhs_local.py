...
MODEL_DIR = Path(r'C:\Projects\aiot\sleepfm-repro\artifacts\embeddings\model_base_local')
...
SPLIT_PATH = Path(r'C:\Projects\aiot\sleepfm-repro\artifacts\splits\shhs_downstream_pilot_split.json')
...
for file_path in tqdm(file_paths, desc='Embedding SHHS local'):
    ...
    try:
        with h5py.File(file_path, 'r') as hf:
            ...
            modality_to_channels = {
                m: [n for n in names if n in channel_groups[m]] for m in modalities
            }
            ...
            with h5py.File(seq_out, 'w') as f_seq, h5py.File(agg_out, 'w') as f_agg:
                for chunk_idx in range(num_chunks):
                    ...
                    for modality in modalities:
                        ...
                        pooled = pooled[0].detach().cpu().numpy().astype(np.float32)   # [128]
                        seq = seq[0].detach().cpu().numpy().astype(np.float32)         # [S, 128]
...
with open(MODEL_DIR / 'shhs_local_embedding_summary.json', 'w', encoding='utf-8') as f:
